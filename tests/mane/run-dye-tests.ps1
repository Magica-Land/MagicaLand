...
$testPrefix = Join-Path $testRepo 'appearance/src/client/java/top/csituka/magicaland/client'
$testSources = @(
...
$testSources += Join-Path $testRepo 'appearance/src/main/java/top/csituka/magicaland/cutiemark/CutieMarkData.java'
...
& java -cp ($testOutput + [IO.Path]::PathSeparator + $testGson) ManeDyeTest (Join-Path $testRepo 'appearance/src/main/resources/assets/magicaland/mane_dyes/stripe01.json')
if ($LASTEXITCODE -ne 0) { throw 'Mane dye tests failed' }
& java -cp ($testOutput + [IO.Path]::PathSeparator + $testGson) ManeDyeRoutingTest (Join-Path $testRepo 'appearance/src/main/resources/assets/magicaland/mane_dyes')
