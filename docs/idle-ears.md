...
`ear_controller` 不再循环播放固定 11 秒的右耳双抽。`PonyIdleEars` 根据玩家 UUID 和世界游戏时间安排耳动。每个 5 秒窗口都有 50% 的概率不发生耳动；发生时，具体时间会错开。在发生的耳动中，左耳、右耳和错峰双耳约占 45%、45% 和 10%。

大多数单抽，约 20% 保留双抽，幅度取作者原动作的 35%／45%／55%，时长小幅变化。两耳都能先动，不保证轮流，不逐帧投随机数。上述全部是可调整的美术初值，不是实测马匹生理频率。
